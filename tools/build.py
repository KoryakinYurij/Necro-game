...
CORE = [
    ...
    # Exploration #2: one user-facing Arena/Exploration selector; legacy finale/endless is no longer the mode control
    ('function n(){t.innerHTML=`РЕЖИМ: <b>`+(d.mode===`finale`?`ФИНАЛ 15 МИН`:`БЕСКОНЕЧНЫЙ`)+`</b>`}',
     'function n(){t.innerHTML=`РЕЖИМ: <b>`+((window.__nxOrchestrationMode||`arena`)===`exploration`?`ИССЛЕДОВАНИЕ`:`АРЕНА`)+`</b>`}'),
    ('t.onclick=()=>{d.mode=d.mode===`finale`?`endless`:`finale`,p(),n(),T(`select`)}',
     't.onclick=()=>{window.__nxOrchestrationMode=(window.__nxOrchestrationMode||`arena`)===`arena`?`exploration`:`arena`,n(),T(`select`)}'),
    # Exploration #2: additive world render seam after the existing floor and before entities
    ('h.fillStyle=`#07040d`,h.fillRect(0,0,g,_),Yn();let n=a.time;',
     'h.fillStyle=`#07040d`,h.fillRect(0,0,g,_),Yn(),(window.__nxOrchestrationMode||`arena`)===`exploration`&&typeof window.__nxExplorationWorldRenderHook===`function`&&window.__nxExplorationWorldRenderHook(h);let n=a.time;'),
    ...
    ('function z(){let e=a&&a.time?a.time/60:0,t=Math.min(j.powerScale.cap*.5,xt()*j.powerScale.dmg);return j.dmgTime(e)*(1+t)}',
     'function z(){let e=a&&a.time?a.time/60:0,t=Math.min(j.powerScale.cap*.5,xt()*j.powerScale.dmg),n=(window.__nxOrchestrationMode||`arena`)===`exploration`?((window.__nxExplorationDifficulty&&window.__nxExplorationDifficulty.dmg)||1):j.dmgTime(e);return n*(1+t)}'),
    ...
    ('castUlt:()=>Vr()}',
     'castUlt:()=>Vr(),save:p,castSpell:Bn,refreshRes:Mn,renderMeta:X,pactDefs:We,relicDefs:He,evoDefs:Ne,specDefs:Ve,weaponDefs:je,isMuted:()=>le,toggleMute:ge,resume:()=>wt(),pause:()=>Ct(),refreshMenu:Rn,w2s:(e,t)=>[e-Wn+v,t-Gn+y],cam:()=>({w:g,h:_}),gfx:{enemy:e=>{er=e},hero:e=>{Zn=e},minion:e=>{Qn=e},golem:e=>{$n=e},floor:e=>{Yn=e},shadow:e=>{Kn=e}},icons:Be,families:Te,summons:Oe,exploration:{get mode(){return window.__nxOrchestrationMode||`arena`},setMode:e=>{if(e!==`arena`&&e!==`exploration`)throw new Error(`invalid orchestration mode`);window.__nxOrchestrationMode=e;let t=document.getElementById(`modeBtn`);t&&(t.innerHTML=`РЕЖИМ: <b>`+(e===`exploration`?`ИССЛЕДОВАНИЕ`:`АРЕНА`)+`</b>`)},setSimulationHook:e=>{if(e!=null&&typeof e!==`function`)throw new TypeError(`simulation hook must be a function or null`);window.__nxExplorationSimHook=e},setWorldRenderHook:e=>{if(e!=null&&typeof e!==`function`)throw new TypeError(`world render hook must be a function or null`);window.__nxExplorationWorldRenderHook=e},spawn:(e,t,n,r=null)=>H(e,t,n,r),get difficulty(){let e=window.__nxExplorationDifficulty||{hp:1,dmg:1};return{hp:e.hp,dmg:e.dmg}},setDifficulty:e=>{if(!e||!Number.isFinite(e.hp)||!Number.isFinite(e.dmg)||e.hp<=0||e.dmg<=0)throw new TypeError(`difficulty requires positive hp and dmg scales`);window.__nxExplorationDifficulty={hp:e.hp,dmg:e.dmg}}}},window.castLegionSpell=Bn'),
    # подсказка в меню
